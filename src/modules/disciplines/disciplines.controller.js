const service = require('./disciplines.service');
...
async function getClasses(req, res, next) {
  try { success(res, await service.getClasses(req.params.id)); } catch (err) { next(err); }
}

async function getEnrollments(req, res, next) {
  try { success(res, await service.getEnrollments(req.params.id, req.query)); } catch (err) { next(err); }
}
